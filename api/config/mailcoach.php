...
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\ButtonRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\CodeRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\DelimiterRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\HeaderRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\ImageRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\ListRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\ParagraphRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\QuoteRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\RawRenderer;
use Spatie\Mailcoach\Domain\Editor\EditorJs\Renderer\TableRenderer;
...
use Spatie\Mailcoach\Http\App\Middleware\BootstrapNavigation;
...
return [
    ...
    'campaigns' => [
        ...
        /**
         * How long send campaign mail jobs can be queued and sitting in the queue
         * before being marked as too many attempts. This also determines when
         * sends are considered "stuck" and are retried.
         */
        'send_campaign_mail_job_retry_until_seconds' => 60 * 60 * 3,

        /**
         * Send models are created in batches to increase efficiency,
         * you can lower or increase the batch size if needed.
         */
        'create_sends_batch_size' => 1_000,
    ...
    'audience' => [
        ...
        /**
         * Which email validation rules Mailcoach should use.
         * By default it uses a strict RFC check + DNS
         */
        'email_validation_rule' => 'email:strict,dns',
    ],
    ...
    /*
     * Set this to true if you use Redis. This will use atomic Redis-based
     * rate limiting for mail jobs, which is more reliable when running
     ...
     * of a list. You should make sure that this disk is
     * publicly reachable.
     */
    'website_disk' => 'public',
    ...
    /**
     * The prefix in the URL we use for list websites
     */
    ...
    'uploads' => [
        ...
        /*
        * Here you can specify the database connection for media's models
        * Leave empty to use the app default's env('DB_CONNECTION')
        */
        'media_database_connection' => '',
    ],
